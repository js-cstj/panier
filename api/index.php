...
$dsn = 'sqlite://../database/aubonrabais.sqlite';
$clients = [];
...
function traiterFILES($array, $table, $name) {
	Output::$default_folder = "/images/" . $table;
	Output::$default_pattern = "%s{name}/%d{width}x%d{height}.%s";
	$result = [];
	foreach ($array as $value) {
		$image = new Image($value['tmp_name']);
		$image->name = $name;
		$image->addOutput(500, 500, "image/webp");
		$image->addOutput(500, 0, "image/webp");
		$image->addOutput(0, 500, "image/webp");
